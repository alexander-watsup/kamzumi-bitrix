						...
						?><tr class="rsec_jsline" data-id="<?=$arItem['ID']?>"><?
							...
							if($ident!='viewed')
							{
								?><td class="rsec_min rsec_cen"><?
									// checkbox
									?><input type="checkbox" name="<?=$arParams['RS_TAB_IDENT']?>_DELETE_<?=$arItem['ID']?>" id="<?=$arParams['RS_TAB_IDENT']?>_DELETE_<?=$arItem['ID']?>" value="Y"><?
                                    ?><label class="rsec_checkbox" for="<?=$arParams['RS_TAB_IDENT']?>_DELETE_<?=$arItem['ID']?>"></label><?
								?></td><?
							}
