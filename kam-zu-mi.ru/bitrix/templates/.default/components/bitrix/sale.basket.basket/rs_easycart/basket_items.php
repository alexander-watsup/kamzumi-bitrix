				...
				if($arItem['DELAY']=='N' && $arItem['CAN_BUY']=='Y') {
					?><tr class="rsec_jsline" data-id="<?=$arItem['ID']?>"><?
						...
						?><td class="rsec_min rsec_cen"><?
							// checkbox
							?><input type="checkbox" name="DELETE_<?=$arItem['ID']?>" id="easycart_DELETE_<?=$arItem['ID']?>" value="Y"><label class="rsec_checkbox" for="easycart_DELETE_<?=$arItem['ID']?>"></label><?
						?></td><?
						...
						?><td class="rsec_delete rsec_min rsec_cen rsec_padd"><?
							?><a class="rsec_delete" href="#easycart_DELETE_<?=$arItem["ID"]?>" title="<?=GetMessage('SALE_DELETE')?>"><i class="rsec_iconka"></i></a><?
						?></td><?
					?></tr><?
				}
			}
