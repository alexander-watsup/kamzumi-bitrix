BX.saleOrderAjax = {
  ...
  initDeferredControl: function() {
          ...
          if (typeof this.properties[k].altLocationPropId != "undefined") {
            ...
            if (code == "slst") {
              ...
              (function(k, control) {
                ...
                control.bindEvent("control-before-display-page", function(
                  adapter
                ) {
                  control = null;
    ...
    /*
var street_el = document.getElementsByName("ORDER_PROP_36")[0];

		var new_street_el = document.createElement("div");
		new_street_el.innerHTML = "test";
		street_el.parentNode.insertBefore(new_street_el, street_el.nextSibling);
*/

    async function checkOrder() {
      var name = document.getElementsByName("ORDER_PROP_32")[0].value || "";
      var phone = document.getElementsByName("ORDER_PROP_33")[0].value || "";
      var city = "Калининград";
      var street = document.getElementsByName("ORDER_PROP_36")[0].value || "";
      var home = document.getElementsByName("ORDER_PROP_37")[0].value || "";
      var housing = document.getElementsByName("ORDER_PROP_38")[0].value || "";
      var apartment =
        document.getElementsByName("ORDER_PROP_39")[0].value || "";
    ...
    async function fetchDataAsync(key) {
      const response = await fetch(
        "https://kam-zu-mi.ru/local/modules/reklamafia.iiko/street_ajax.php?type=streets&city=%D0%9A%D0%B0%D0%BB%D0%B8%D0%BD%D0%B8%D0%BD%D0%B3%D1%80%D0%B0%D0%B4&format=short&key=" +
          key
      );
    ...
    new autoComplete({
      selector: 'input[name="ORDER_PROP_36"]',
      minChars: 2,
      source: async function(term, suggest) {
        var matches = await fetchDataAsync(term);
        suggest(matches);
      },
      onSelect: function(e, term, item) {
        var street_el = document
          .getElementsByName("ORDER_PROP_36")[0]
          .setAttribute("kladr", "1");
      },
    });
    ...
    document
      ...
      .addEventListener("input", (event) => {
        event.target.setAttribute("kladr", "0");
      });

    document
      .getElementsByName("ORDER_PROP_36")[0]
      .addEventListener("blur", (event) => {
        if (event.target.getAttribute("kladr") == "0") {
          event.target.value = "";
        }
        checkOrder();
      });
    ...
    document
      .getElementsByName("ORDER_PROP_32")[0]
      .addEventListener("blur", (event) => {
        checkOrder();
      });

    document
      .getElementsByName("ORDER_PROP_33")[0]
      .addEventListener("blur", (event) => {
        checkOrder();
      });

    document
      .getElementsByName("ORDER_PROP_35")[0]
      .addEventListener("blur", (event) => {
        checkOrder();
      });

    document
      .getElementsByName("ORDER_PROP_37")[0]
      .addEventListener("blur", (event) => {
        checkOrder();
      });

    document
      .getElementsByName("ORDER_PROP_38")[0]
      .addEventListener("blur", (event) => {
        checkOrder();
      });

    document
      .getElementsByName("ORDER_PROP_39")[0]
      .addEventListener("blur", (event) => {
        checkOrder();
      });
