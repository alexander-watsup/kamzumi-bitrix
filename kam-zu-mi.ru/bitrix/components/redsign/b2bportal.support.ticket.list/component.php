...
$arResult["FILTER"] = array(
	...
	array("id"=>"UF_ORDER_ID", "name"=>GetMessage('SUP_F_ORDER_ID')),
...
if (isset($arParams['UF_ORDER_ID']))
{
	$aFilter['UF_ORDER_ID'] = $arParams['UF_ORDER_ID'];
}
elseif (isset($aFilter['UF_ORDER_ID']) && strlen($aFilter['UF_ORDER_ID']) > 0)
{
	global $USER;

	$accountNumber = $aFilter['UF_ORDER_ID'];
	
	$order = Bitrix\Sale\Order::loadByAccountNumber($accountNumber);
	if ($order && $USER->GetId() == $order->getUserId())
	{
		
		$aFilter['UF_ORDER_ID'] = $order->getId();
	}
}
...
$arResult["NAV_STRING"] = $rsTickets->GetPageNavString(GetMessage("SUP_PAGES"), $arParams['NAV_TEMPLATE'] ?? '');
